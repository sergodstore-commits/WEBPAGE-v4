ALTER TABLE public.pos_sales DROP CONSTRAINT pos_sales_delivery_shape_ck;
ALTER TABLE public.pos_sales ADD CONSTRAINT pos_sales_delivery_shape_ck CHECK (
...
CREATE OR REPLACE FUNCTION public.sergod_validate_pos_completion()
  ...
  IF NEW.state='COMPLETED' AND OLD.state<>'COMPLETED' THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.pos_sale_settlements s WHERE s.pos_sale_id=NEW.pos_sale_id
        ...
        AND ((NEW.total_amount_clp=0 AND s.kind='ZERO_TOTAL')
          OR (NEW.total_amount_clp>0 AND s.kind='EXTERNAL_DECLARED'))
    ...
    IF NEW.sale_type='REGULAR' AND EXISTS (
      SELECT 1 FROM public.pos_sale_lines l WHERE l.pos_sale_id=NEW.pos_sale_id AND NOT EXISTS (
        ...
        WHERE m.source_type='POS_SALE' AND m.source_id=NEW.pos_sale_id::text
          AND m.movement_type='POS_SALE_CONSUMED' AND ip.product_id=l.product_id
          AND ip.branch_id=NEW.branch_id AND m.quantity=l.quantity
      )
    ...
    IF NEW.sale_type='PREORDER' AND (
      ...
      OR (NEW.account_id IS NULL AND (NEW.buyer_name IS NULL
        OR (NEW.buyer_email IS NULL AND NEW.buyer_phone IS NULL)))
    ...
    IF NEW.delivery_mode='SHIPPING' AND (
      ...
      OR NEW.delivery_snapshot->'shippingCostAmountClp'<>'0'::jsonb
    ...
    IF NEW.sale_type='PREORDER' AND EXISTS (
      SELECT 1 FROM public.pos_sale_lines l JOIN public.products p ON p.product_id=l.product_id
      WHERE l.pos_sale_id=NEW.pos_sale_id AND (p.sale_type<>'PREORDER'
        OR l.preorder_campaign_id IS NULL OR NOT EXISTS (
          SELECT 1 FROM public.preorder_commitments c WHERE c.pos_sale_line_id=l.pos_sale_line_id
            AND c.state='PAID_COMMITTED'
        ))
...
REVOKE EXECUTE ON FUNCTION public.sergod_validate_pos_completion() FROM PUBLIC;
