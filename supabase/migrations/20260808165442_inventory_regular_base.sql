CREATE TABLE public.system_configurations (
  ...
  activated_by uuid REFERENCES public.user_accounts(account_id),
  activated_at timestamptz,
  retired_by uuid REFERENCES public.user_accounts(account_id),
...
CREATE UNIQUE INDEX system_configurations_active_global_idx
  ON public.system_configurations (configuration_key, scope)
  WHERE state = 'ACTIVE' AND scope = 'GLOBAL';
CREATE UNIQUE INDEX system_configurations_active_branch_idx
  ON public.system_configurations (configuration_key, scope, branch_id)
  WHERE state = 'ACTIVE' AND scope = 'BRANCH';
CREATE UNIQUE INDEX system_configurations_global_version_idx
  ON public.system_configurations (configuration_key, scope, version_number)
  WHERE scope = 'GLOBAL';
CREATE UNIQUE INDEX system_configurations_branch_version_idx
  ON public.system_configurations (configuration_key, scope, branch_id, version_number)
  WHERE scope = 'BRANCH';
...
CREATE INDEX inventory_positions_branch_idx
  ON public.inventory_positions (branch_id, product_id);
...
CREATE INDEX inventory_movements_position_history_idx
  ON public.inventory_movements (inventory_position_id, occurred_at DESC, movement_id DESC);

CREATE TRIGGER inventory_movements_immutable
  BEFORE UPDATE OR DELETE ON public.inventory_movements
  FOR EACH ROW EXECUTE FUNCTION public.sergod_prevent_change();

CREATE OR REPLACE FUNCTION public.sergod_catalog_protect_sale_type()
...
REVOKE ALL ON TABLE public.system_configurations, public.inventory_positions,
  public.inventory_movements FROM PUBLIC;
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'service_role') THEN
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.system_configurations,
      public.inventory_positions, public.inventory_movements TO service_role;
  END IF;
END;
$$;
