...
import { CartExpirationJob } from '../apps/api/src/contexts/commerce-orders/application/cart-expiration-job.js';
import { PgCartRepository } from '../apps/api/src/contexts/commerce-orders/infrastructure/postgres-cart-repository.js';
...
try {
  ...
  const result = await new CartExpirationJob(
    new PgCartRepository(pool, clock, uuids),
    coordination,
    clock,
  ).run({ correlationId: uuids.generate(), scheduledFor });
  process.stdout.write(`Anonymous cart expiration job ${result.kind.toLowerCase()}.\n`);
} finally {
  await pool.end();
}
