...
import { PreorderLifecycleJob } from '../apps/api/src/contexts/preorders/application/preorder-lifecycle-job.js';
import { PgPreordersRepository } from '../apps/api/src/contexts/preorders/infrastructure/postgres-preorders-repository.js';
...
try {
  ...
  const result = await new PreorderLifecycleJob(
    new PgPreordersRepository(pool, clock, uuids),
    coordination,
    clock,
  ).run({ correlationId: uuids.generate(), scheduledFor });
  process.stdout.write(`Preorder lifecycle job ${result.kind.toLowerCase()}.\n`);
} finally {
  await pool.end();
}
